...
func TestValidateMsg(t *testing.T) {
	...
	t.Run("valid", func(t *testing.T) {
		msg := types.SSVMessage{
			MsgID:   types.MessageIDForValidatorPKAndRole(types.ValidatorID{1, 2, 3, 4}, beacon.RoleTypeAttester),
	...
	t.Run("wrong msg id", func(t *testing.T) {
		msg := types.SSVMessage{
			MsgID: types.MessageIDForValidatorPKAndRole(types.ValidatorID{1, 2, 3, 3}, beacon.RoleTypeAttester),
		}
	...
	t.Run("no duty runner", func(t *testing.T) {
		msg := types.SSVMessage{
			MsgID: types.MessageIDForValidatorPKAndRole(types.ValidatorID{1, 2, 3, 4}, beacon.RoleTypeAggregator),
		}
